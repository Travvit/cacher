...
async function waitUntilMsg(msg, gcProc) {
    return new Promise(async (resolve, reject) => {
        const receivedMsg = await getGCMessage(gcProc);
        if (receivedMsg === msg) {
            resolve(receivedMsg);
        } else {
            waitUntilMsg(msg, gcProc);
        }
    });
}
...
describe('CachedObjectFactory', () => {
    ...
    describe.only('Garbage collection', async () => {
        ...
        it.only('Receives the start message', async () => {
            // let result = await getGCMessage(subProc);
            let result = await waitUntilMsg('GC process started.', subProc);
            expect(result).to.contain('GC process started.');
        });

        it.only('Load a bunch of data and have them expired', async () => {
            if (redisRWClient.connected) {
                // Get the key
                const key = hashFactory.create('TestClass', 'unconfiguredMethod', ['world']);
                // Call the proxied method
                ...
                val = await existsAsync(key);
                expect(val).to.equal(0);
                // Wait for a bit and ensure GC took place.
                result = await waitUntilMsg('GC started.', subProc);
                await sleep(1000);
                val = await existsAsync(`BUCKET.${APP_ENV}.${APP_NAME}.GLOBAL`);
                expect(val).to.equal(0);
            }
        }).timeout(300 * 1000);

        it.only('Load a bunch of data using TTL method and have them expired', async () => {
            if (redisRWClient.connected) {
                // Get the key
                const key = hashFactory.create('TestClass', 'ttlMethod', ['world']);
                // Call the proxied method
        ...
        it.only('Load a bunch of data using TTL method with multiple buckets and have them expired', async () => {
            if (redisRWClient.connected) {
                // Get the key
                const key = hashFactory.create('TestClass', 'ttlBucketsMethod', ['world']);
                // Call the proxied method
