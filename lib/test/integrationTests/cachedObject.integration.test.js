...
const hgetallAsync = promisify(redisRWClient.hgetall).bind(redisRWClient);
...
const sismemberAsync = promisify(redisRWClient.sismember).bind(redisRWClient);
const spopAsync = promisify(redisRWClient.spop).bind(redisRWClient);
...
describe('CachedObject', async () => {
    ...
    before(async () => {
        ...
        // redisPSClient.psubscribe('__keyevent@0__:expired'); // Key expired
        // redisPSClient.psubscribe('__keyevent@0__:del'); // Key deleted
        // redisPSClient.psubscribe('__keyevent@0__:sadd'); // Add to Set
        // redisPSClient.psubscribe('__keyevent@0__:srem'); // Remove from Set
        // redisPSClient.psubscribe('__keyevent@0__:hset'); // Hash/Dictionary saved
        // redisPSClient.psubscribe('__keyevent@0__:hincrby'); // Increment Hash/Dictionary value by
    });
    beforeEach(async () => {
    });
    describe('#cachify', async () => {
        ...
        it('Get a cached object from the cacher, no options.', async () => {
            assert.containsAllKeys(cachedObject, ['options', 'getOptions', 'cacheHandler', 'passthroughHandler', 'mutatorHandler', 'cachedObject']);
        });
