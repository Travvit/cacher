...
describe('CachedObject', async () => {
    ...
    redisPSClient.on('pmessage', async (pattern, channel, message) => {
        ...
        switch (channel.match(/__keyevent@\d+__:(.+)/)[1]) {
            case 'del':
                if (message.startsWith('BUCKET-tz-cacher-dev-bucket-1')) {
    ...
    describe('#cachify', async () => {
        ...
        describe('Runs when Redis is online!', async () => {
            ...
            it('Cacher caches a TTL method with buckets.', async () => {
                if (redisRWClient.connected) {
                    // redisRWClient.info('all', (err, res) => {
                    //     console.log(res);
                    // });
            ...
            it.only('Caches a lot of data and expires them.', async () => {
                ...
                if (redisRWClient.connected) {
                    ...
                    console.log(`Memory used: ${getMemUsage(await infoAsync('memory'))}`);
                    timer.interval();
                    console.log('-------------------------------------------------------------------------');
                    ...
                    cachedObject.bucketMethodMutator1('test');
                    // Wait for - Read Time
