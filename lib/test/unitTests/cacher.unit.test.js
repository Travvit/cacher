...
describe('Cacher', () => {
    const cacher = new Cacher(CacheManagerStub);
    ...
    describe('#setCachedValue', async () => {
        it('Returns error when Redis is not available.', async () => {
            try {
                await cacher.setCachedValue('REDIS_UNAVAILABLE');
            } catch (error) {
                expect(error).to.be.an('error');
                expect(error.message).to.contain('Redis is not available.');
                return;
        ...
        it('Returns OK with valid parameters.', async () => {
            const result = await cacher.setCachedValue('TestClass', 'testMethod', 'testKey', { name: 'tester', skill: 95 }, 500, ['test-bucket']);
            expect(result).to.be.a('string');
