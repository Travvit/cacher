...
class RedisGarbageCollector extends GarbageCollector {
    ...
    async run(gcTimer) {
        try {
            ...
            if (res) {
                logger.log(chalk.red('I am the master!!!'));
                ...
                logger.log(chalk.green('I am just a follower...'));
            }
        ...
        // Reschedule GC
        this.gcTimer = setTimeout(this.run.bind(this), 1000, this.gcTimer);
    }
...
================================================
    ...
    async [checkCPULoad]() {
        return new Promise(async (resolve, reject) => {
            ...
            // logger.log(`Current CPU usage is too high: ${currentLoad.currentload}`);
            // logger.log(`Max CPU: ${this.CPU_LOAD_CUTOFF}`);
            if (currentLoad.currentload < this.CPU_LOAD_CUTOFF) {
                // logger.log('CPU Usage good');
                resolve(true);
            } else {
                // logger.log('CPU Usage bad');
                // reject(`Current CPU usage is too high: ${currentLoad.currentload}`);
                reject(false);
            }
        });
    }
    ...
    async [executeGC]() {
        ...
        if (buckets.length) {
            ...
            _.forEach(buckets, async (bucket) => {
                bucketList.push({ bucket, size: await this[scardAsync](bucket) });
            });
