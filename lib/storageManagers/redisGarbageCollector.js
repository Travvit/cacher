        ...
        if (buckets.length) {
            ...
            for (let bucket of buckets) {
                ...
                if (!bucket.endsWith('GLOBAL')) {
                    let size = await this[scardAsync](bucket);
                    let bucketItem = { bucket, size };
            ...
            //     // Skip GLOBAL buckets
            //     if (!bucket.endsWith('GLOBAL')) {
            //         let bucketItem = { bucket, size: await this[scardAsync](bucket) };
            //         bucketList.push(bucketItem);
            //     }
            // });
            logger.log('List of buckets:');
            logger.log(bucketList);
            // Sort based on bucket size
            bucketList.sort((first, second) => first.size - second.size);
            logger.log(JSON.stringify(bucketList));
            // Clean the buckets.
            ...
            try {
                ...
                local cleanBucketFn = function(myBucket)
                    ...
                    for i = 1, table.getn(bucketMembers), 1 do
                        ...
                        if redis.call('EXISTS', bucketMembers[i]) then
...
process.on('exit', gc.shutdown.bind(gc));
