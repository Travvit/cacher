...
class RedisStorageManager extends StorageManager {
    constructor() {
        ...
        setInterval(this[collectGarbage].bind(this), 15000); // Collect garbage every 15 seconds
    }
    ...
    */
    ...
    async [collectGarbage]() {
        if (this[checkRedisAvailable]()) {
            ...
            try {
                const { stdout, stderr } = await execFile('node', [`${__dirname}/redisGarbageCollector.js`], { timeout: 5000 });
                logger.log(stdout);
                logger.log('-----------------------');
                // logger.log(stderr);
            } catch (error) {
