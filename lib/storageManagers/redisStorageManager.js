...
class RedisStorageManager extends StorageManager {
    constructor() {
        ...
        this[PUB_SUB_CONN].subscribe(`${NODE_ENV}:bucket_del`); // Channel to announce the deletion of a bucket
        this[PUB_SUB_CONN].subscribe(`${NODE_ENV}:cacher_error`); // Channel to cacher errors
    ...
    async [init]() {
        try {
            ...
            local deleteBucketMembersFn = function(myBucket)
                ...
                if result == 1 then
                    redis.call('PUBLISH', "${NODE_ENV}:bucket_del", myBucket);
                end;
            end;
                ...
                for key, bucket in pairs(buckets) do
                    ...
                    if (table.getn(bucketAppNameTable) > 1) then
                        ...
                        if (bucketApp == APP_NAME) then
                            ...
                            redis.call('PUBLISH', "${NODE_ENV}:cacher_error", 'WARNING: Cannot purge bucket of a different app.');
                        end;
                    else
                        -- No App specified for bucket
                        -- Find the buckets associated with the other app
    ...
    async [handleRedisConnect]() {
        ...
        process.on('exit', () => {
            if (this[garbageCollector]) {
                process.kill(this[garbageCollector].pid);
            }
        });
    }
