...
/* The interval for the Garbage Collection events to trigger. */
const GC_INTERVAL_MIN = 15000;
const GC_INTERVAL = (process.env.REDIS_GC_INTERVAL && (parseInt(process.env.REDIS_GC_INTERVAL, 10) > GC_INTERVAL_MIN)) ? parseInt(process.env.REDIS_GC_INTERVAL, 10) : GC_INTERVAL_MIN;
/* The Garbage Collection Process timeout. Assume 1 second to process setInterval */
const GCP_TIMEOUT = GC_INTERVAL - 1000;
...
const collectGarbage = Symbol('collectGarbage');
...
class RedisStorageManager extends StorageManager {
    constructor() {
        ...
        // setInterval(this[collectGarbage].bind(this), GC_INTERVAL); // Collect garbage every 15 seconds
    }
    ...
    */
    ...
    async [collectGarbage]() {
        ...
        if (this[checkRedisAvailable]()) {
            logger.log(`[${new Date()}] Collecting garbage...`);
            try {
                const { stdout } = await execFile('node', [`${__dirname}/redisGarbageCollector.js`], { timeout: GCP_TIMEOUT });
                logger.log(stdout);
                logger.log('-----------------------');
                // logger.log(stderr);
            } catch (error) {
    ...
    async [init]() {
        try {
            ...
            -- redis.call('SADD', 'BUCKET.' .. APP_ENV .. '.tz-permission.bucket-1', 1);
            -- redis.call('SADD', 'BUCKET.' .. APP_ENV .. '.tz-permission.bucket-2', 2);
            -- redis.call('SADD', 'BUCKET.' .. APP_ENV .. '.tz-permission.bucket-3', 3);
            --------------------------
                ...
                for key, bucket in pairs(buckets) do
                    ...
                    if (table.getn(bucketAppNameTable) > 1) then
                        ...
                        if (bucketApp == APP_NAME) then
                            ...
                            --break;
                            --if (bucketName == '*') then
                            --    -- Ensure the no one is allowed to completely purge the cache of a different app/lib
                            --    print('WARNING: Completely purging cache of a different app is not allowed.')
                            --    break;
                            --end;
                            --print('Purging buckets of app: ' .. bucketApp);
                            ---- Find the buckets associated with the other app
                            --result = redis.call('KEYS', 'BUCKET.' .. APP_ENV .. '.' .. bucketApp .. '.' .. bucketName);
                            --for x, otherAppBucket in pairs(result) do
                            --    print('Purging keys from bucket: ' .. otherAppBucket);
                            --    -- redis.call('SADD', otherAppBucket, key);
                            --end;
                        end;
                    else
            ...
            if (count == 0) then
                ...
                -- remove GLOBAL bucket
                result = redis.call('DEL', globalBucket);
                --redis.call('PUBLISH', APP_ENV .. ":bucket_del", globalBucket);
                return 1;
            end;
            ...
            print('Size of GLOBAL bucket: ' .. redis.call('SCARD', globalBucket));
            -- Delete the remaining keys
            if table.getn(deletedMembers) > 0 then
                print(unpack(deletedMembers));
                result = result + redis.call('SREM', globalBucket, unpack(deletedMembers));
