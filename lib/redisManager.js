    ...
    */
    ...
    async purgeBuckets(buckets) {
        if (this[checkRedisAvailable]()) {
            ...
            try {
                this[sendCommandAsync]('EVALSHA', [this[purgeBucketsSha1], 1, this.BUCKET_PREFIX, buckets.toString()]);
    ...
    async [init]() {
        ...
        try {
            await this[configAsync]('SET', 'notify-keyspace-events', 'EA');
            ...
            if buckets ~= nil then
                for key, value in pairs(buckets) do
                    ...
                    for key, val in pairs(bucketMembers) do
                        -- removedKey = removedKey .. ' ' .. val;
                        -- Delete key from cache
                        -- removedKeyList = removedKeyList .. string.gsub(val, '%"', '') .. ' ';
    ...
    async [removeKey](key) {
        ...
        try {
            const result = await this[sendCommandAsync]('EVALSHA', [this[removeKeySha1], 1, key, this.BUCKET_PREFIX]);
            return result;
        } catch (error) {
