...
/* Constants and flags */
const APP_NAME = process.env.APP_NAME || 'tz-permissions';
...
class RedisManager {
    ...
    */
     ...
     */
    [createConnection]() {
        const MAX_RETRY_TIME = 30 * 1000; // 30 seconds
        ...
        return (NODE_ENV !== 'development') ? redis.createClient(REDIS_URL) : redis.createClient({
            ...
            retry_strategy(options) {
                ...
                if (options.total_retry_time > MAX_RETRY_TIME) {
                    ...
                    logger.error('Connection lost with Redis for more than 30 seconds. Retry time exhausted!');
                }
