...
class Cacher {
    ...
    async setCachedValue({ key, cachedObject, cachedMethod, value, ttl, buckets } = {}) {
        // console.log({ app: this.APP_NAME, env: this.APP_ENV, key, cachedObject, cachedMethod, value, ttl, buckets });
        return this.storageManager.setCachedValue({ app: this.APP_NAME, env: this.APP_ENV, key, cachedObject, cachedMethod, value, ttl, buckets });
    }
    ...
    async purgeBuckets(buckets) {
        return this.storageManager.purgeBuckets({ app: this.APP_NAME, env: this.APP_ENV, buckets });
    }
}
