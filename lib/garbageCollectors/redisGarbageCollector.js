...
// Flags
const shuttingDown = Symbol('shuttingDown')
...
================================================
    ...
    async shutdown(reason) {
        if (this[shuttingDown]) {
            logger.log(`Ignoring ${reason}, because GC is shutting down.`);
            return;
        }
        this[shuttingDown] = true;
        logger.log(`${reason}: GC timed out, or killed!`);
        // Release lock
        await this[releaseLock]('gc_lock', this.key);
        timer.intervalMS();
        process.exit(0);
    }
    ...
    async [run](gcTimer) {
        try {
            ...
            if (await this[checkCPULoad]()) {
                this.key = await this[generateGCSessionKey]();
                leaseReqResult = await this[getOrRenewLeaseLock]('gc_lock', this.key, this.GCM_LEASE_DURATION);
            }
        ...
        return redis.createClient({
            ...
            retry_strategy(options) {
                if (options.error && options.error.code === 'ECONNREFUSED') {
                    ...
                    this[send]('The server refused the connection!');
                }
                if (options.total_retry_time > this.MAX_RETRY_TIME) {
                    ...
                    this[send](`Connection lost with Redis for more than ${this.MAX_RETRY_TIME / 1000} seconds. Retry time exhausted!`);
                    return null;
                }
                if (options.attempt > this.MAX_RETRY_ATTEMPTS) {
                    ...
                    this[send](`Maximum connection retry attempts (${this.MAX_RETRY_ATTEMPTS}) exhausted!`);
                    return null;
                }
                // reconnect after
    ...
    async [releaseLock](resource, value) {
        // TODO: Return a promise and reject lock release after a given timeout.
        if (this[checkRedisAvailable]()) {
            try {
...
const argv = require('yargs/yargs')(process.argv.slice(2))
    .options({
        ...
        REDIS_URL: {
            ...
            default: process.env.REDIS_URL || process.env.REDISCLOUD_URL || 'redis://localhost:6379',
            type: 'string'
        },
        REDIS_MANAGER_ON: {
 ...
 */
process.on('SIGTERM', gc.shutdown.bind(gc, 'SIGTERM'));
process.on('SIGINT', gc.shutdown.bind(gc, 'SIGINT'));
