...
const cleanBucket = Symbol('cleanBucket'); // Need to implement properly
...
class RedisGarbageCollector extends GarbageCollector {
    ...
    async loadOptions(argv) {
        ...
        this.REG_BUCKET_MEMBER_BULK_DEL_SIZE = argv.REG_BUCKET_MEMBER_BULK_DEL_SIZE;
    }
...
================================================
    ...
    async [run](gcTimer) {
        try {
            ...
            if (res) {
                logger.log(chalk.red('GC master status acquisition success.'));
                ...
                logger.log(chalk.green('GC master status acquisition failed.'));
            }
            logger.log(`Sleeping for: ${this.GCM_LEASE_DURATION}`);
    ...
    async [executeGC]() {
        ...
        if (buckets.length) {
            ...
            buckets.forEach(async (bucket, index) => {
                // Skip GLOBAL buckets
                if (!bucket.endsWith('GLOBAL')) {
                    bucketsList.push({ bucket, size: bucketSizes[index] });
                }
            });
            logger.log('List of buckets:');
            logger.log(bucketsList);
            // Sort based on bucket size
            bucketsList.sort((first, second) => first.size - second.size);
            logger.log(JSON.stringify(bucketsList));
            // Clean the buckets.
    ...
    async [cleanBucket](bucket) {
        ...
        if (this[checkRedisAvailable]()) {
            try {
                ...
                local getGlobalBucketName = function(myBucket)
                    print('- getGlobalBucketName -----------------------------');
                    local bucketNameTable = {}; -- pair(BUCKET, env, app, bucket)
                    for curr in string.gmatch(bucket, '[^%.]+') do
                        table.insert(bucketNameTable, curr);
                    end;
                    local globalBucket = bucketNameTable[1] .. '.' .. bucketNameTable[2] .. '.' .. bucketNameTable[3] .. '.GLOBAL';
                    print(globalBucket);
                    return globalBucket;
                end;

                local removeKeysFromGlobalBucket = function(globalBucket, keys)
                    print('-- removeKeysFromGlobalBucket-------------------------')
                    local cursor = 0;
                    local count = redis.call('SCARD', globalBucket);
                    local deletedMembers = {};
                    -- Go through the individual keys and find the matching global entry
                ...
                local cleanBucketFn = function(myBucket)
                    ...
                    if redis.call('SCARD', myBucket) == 0 then
                        ...
                        if result == 1 then
                            redis.call('PUBLISH', "${this.APP_ENV}:bucket_del", myBucket);
                        end;
                    end;
                end;
                ...
                `;
                const result = await this[sendCommandAsync]('EVAL', [cleanBucketCmd, 0, bucket]);
                await timer.sleep(1000); // * Simulate bucket purge
                logger.log(result);
                return result;
            } catch (error) {
    ...
    async [getOrRenewLeaseLock](resource, value, duration) {
        ...
        // logger.log(`Resource: ${resource}, Value: ${value}, Duration: ${duration}`);
        if (this[checkRedisAvailable]()) {
            ...
            try {
                ...
                `;
                // logger.log(`NODE_ENV: ${this.NODE_ENV}`);
    ...
    async [releaseLock](resource, value) {
        ...
        if (this[checkRedisAvailable]()) {
            try {
                ...
                `;
                const result = 1;// = await this[sendCommandAsync]('EVAL', [releaseLockCmd, 0, this.NODE_ENV, resource, value]);
                return result;
            } catch (error) {
                logger.log(error.message);
                return null;
...
const argv = require('yargs/yargs')(process.argv.slice(2))
    .options({
        ...
        REG_BUCKET_MEMBER_BULK_DEL_SIZE: {
            describe: 'Number of members deleted in bulk from a regular bucket.',
            default: process.env.REG_BUCKET_MEMBER_BULK_DEL_SIZE ? parseInt(process.env.REG_BUCKET_MEMBER_BULK_DEL_SIZE, 10) : 2000,
            type: 'number'
        }
    })
    .help()
