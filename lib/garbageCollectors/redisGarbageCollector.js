...
================================================
    ...
    async [executeGC]() {
        try {
            ...
            const buckets = await this[getBuckets](this.APP_ENV);
            if (buckets.length) {
    ...
    async [getOrRenewLeaseLock](resource, value, duration) {
        if (this[checkRedisAvailable]()) {
            try {
                ...
                `;
                const result = await this[sendCommandAsync]('EVAL', [getOrRenewLeaseLockCmd, 0, this.APP_ENV, resource, value, duration]);
    ...
    async [releaseLock](resource, value) {
        if (this[checkRedisAvailable]()) {
            try {
                ...
                `;
                const result = await this[sendCommandAsync]('EVAL', [releaseLockCmd, 0, this.APP_ENV, resource, value]);
                return result;
            } catch (error) {
                logger.log(error.message);
                return null;
...
const argv = require('yargs/yargs')(process.argv.slice(2))
    .options({
        ...
        GLOBAL_BUCKET_MEMBER_BULK_GC_SIZE: {
            describe: 'Number of members deleted in bulk from a regular bucket.',
            default: process.env.GLOBAL_BUCKET_MEMBER_BULK_GC_SIZE ? parseInt(process.env.GLOBAL_BUCKET_MEMBER_BULK_GC_SIZE, 10) : 250,
            type: 'number'
        }
    })
    .help()
