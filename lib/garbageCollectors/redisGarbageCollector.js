        ...
        if (this[checkRedisAvailable]()) {
            try {
                ...
                local removeKeysFromGlobalBucket = function(globalBucket, keys)
                    ...
                    print(globalBucket);
                    local cursor = 0;
                    local deletedGlobalMembers = {};
                    local count = redis.call('SCARD', globalBucket);
                    -- Ensure GLOBAL bucket is not empty
                    if count == 0 then
                        return 1;
                    end;
                    ...
                    for i = 1, table.getn(keys), 1 do
                        ...
                        table.insert(deletedGlobalMembers, bucketMember[1]);
                    end;
                    print('Error here...');
                    redis.call('SREM', globalBucket, unpack(deletedGlobalMembers));
                    result = redis.call('SCARD', globalBucket);
                    if result == 0 then
                        redis.call('PUBLISH', "${this.NODE_ENV}:bucket_del", globalBucket);
                    end;
                end;
