...
class CacheAssembler {
     ...
     */
    getStorageManager({ type, app, env, instance }) {
        let StorageManager;
        switch (type) {
            case 'Redis':
        ...
        return new StorageManager({ app, env, instance });
    }
    ...
    getCacher(app, env, storageName) {
        ...
        return new Cacher(app, env, this.getStorageManager({ type: storageName, app, env, instance: INSTANCE }));
    }
